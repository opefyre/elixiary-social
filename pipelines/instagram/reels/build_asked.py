...
def write_html(r, workdir, timing):
    tpl = open(os.path.join(HERE, "asked.html")).read()
    ...
    shutil.copy(os.path.join(core.ASSETS, "logo.png"), os.path.join(assets, "logo.png"))
...
def main():
    ...
    subprocess.run(["node", os.path.join(HERE, "capture.js"), html, frames, "--fps", str(fps),
                    "--duration", str(core.DUR), "--chrome", find_chrome()] + (["--guides"] if a.guides else []),

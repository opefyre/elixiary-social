// render.mjs (Elixiary copy of the Finkavo skit engine) — node render.mjs reels/<id>.reel.mjs [--stills 0,2.5,...] [--sheet] [--check] [--workers 4] [--force]
...
const ASSETS = process.env.ASSET_ROOT || path.resolve(HERE, "assets");
const FONTS = path.resolve(HERE, "../render/fonts");
...
const b64 = async (f, mime) => `data:${mime};base64,${(await readFile(f)).toString("base64")}`;
// Elixiary type: Plus Jakarta Sans for everything bold, Inter for small print. The engine's CSS names
// "Noto Sans", so PJS is registered under that name rather than editing every reel.
const fonts = { pjs: await b64(path.join(FONTS, "PlusJakartaSans-latin.woff2"), "font/woff2"), inter: await b64(path.join(FONTS, "Inter-latin.woff2"), "font/woff2") };
const logo = await b64(path.join(ASSETS, "logo.png"), "image/png");
// meta.images = { name: "cutouts/x.webp" } (paths under skits/assets/) become data URIs the reel can draw with E.img
...
@font-face{font-family:"Noto Sans";font-weight:200 900;src:url("${fonts.pjs}") format("woff2")}
@font-face{font-family:"Inter";font-weight:100 900;src:url("${fonts.inter}") format("woff2")}
...
async function open(browser) {
  ...
  if (iconsSrc) await page.addScriptTag({ content: iconsSrc });
  await page.evaluate(async () => { await Promise.all([document.fonts.load('900 40px "Noto Sans"'), document.fonts.load('800 40px "Noto Sans"'), document.fonts.load('400 40px "Inter"')]); await document.fonts.ready; });
